...
def getDataForPlz(plz):
    ...
    for x in nList:
        ...
        c.save(websitename="wetterdienst",url=x["url"], timestamp = time.time(), timestamppred = datetime.datetime.strptime(x["date"], "%d.%m.%Y").timestamp(),postleitzahl=plz, stadt="", mintemperatur= x["mintemp"], maxtemperatur= x["maxtemp"], websitename="WETTERDIENST")
