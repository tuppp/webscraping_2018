...
def getDataForPlz(plz):
    ...
    for x in nList:
        ...
        c.save(url=x["url"], timestamp = time.time(), timestamppred = datetime.datetime.strptime(x["date"], "%d.%m.%Y").timestamp(), postleitzahl=plz, mintemperatur= x["mintemp"], maxtemperatur= x["maxtemp"], websitename="WETTERDIENST")
