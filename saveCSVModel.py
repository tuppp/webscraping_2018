...
class SaveData:
    ...
    def save(self, websitename, url, timestamp, timestamppred, postleitzahl=None, stadt=None, temperatur=None,
             niederschlagswahrscheinlichkeit=None, niederschlagsmenge=None, niederschlag=None,
             windgeschwindigkeit=None, luftdruckground=None, luftdrucksea=None, mintemperatur=None, maxtemperatur=None,
             sonnenstunden=None, bewoelkung=None):
        """
        ...
        try:
            ...
            if temperatur is None:
                if mintemperatur is None:
                        if maxtemperatur is None:
            ...
            if (maxtemperatur is None and type(mintemperatur) == float)\
                    or (mintemperatur is None and type(maxtemperatur) == float):
                raise Exception("Max und Mintemperatur koennen nur paarweise existieren.")

            if stadt is not None and type(stadt) != str:
            ...
            if niederschlag is not None and type(niederschlag) != str:
            ...
            # create filename
            filename = "data/" + str(stamp.tm_mday) + "-" + str(stamp.tm_mon)
            filename = filename + "-" + str(stamp.tm_year) + "_" + websitename
            ...
            if self.filename != filename:
                ...
                if os.path.exists(filename):
                    ...
                    self.csvwriter.writerow(["url", "timestamp", "timestamppred", "postleitzahl", "stadt", "temperatur",
                                             "niederschlagswahrscheinlichkeit", "niederschlagsmenge", "niederschlag",
                                             "windgeschwindigkeit", "luftdruckground", "luftdrucksea", "mintemperatur",
                                             "maxtemperatur", "sonnenstunden", "bewoelkung"])
                    self.csvwriter = csv.writer(self.csvfile, delimiter=',', quoting=csv.QUOTE_ALL)
...
def example():
    ...
    """
    c = SaveData()
    c.save("googlecom", "http://www.google.de", timestamp=time.time(), timestamppred=time.time(), postleitzahl="61231",
           stadt="Berlin",  maxtemperatur=23.4, niederschlagswahrscheinlichkeit=80.20, windgeschwindigkeit=200.0,
           mintemperatur=10.0)
    c.save("googlecom", "http://www.google.de", timestamp=time.time(), timestamppred=time.time(), postleitzahl="61231",
           stadt="Berlin",  maxtemperatur=23.4, niederschlagswahrscheinlichkeit=80.20, windgeschwindigkeit=200.0,
           mintemperatur=10.0)
